...
int press_1, press_2, press_3, press_4;

#define MPU6050_AUX_VDDIO          0x01   // R/W
#define MPU6050_SMPLRT_DIV         0x19   // R/W
#define MPU6050_CONFIG             0x1A   // R/W
#define MPU6050_GYRO_CONFIG        0x03   // R/W
#define MPU6050_ACCEL_CONFIG       0x04   // R/W
...
#define MPU6050_EXT_SENS_DATA_11   0x54   // R  #define MPU6050_EXT_SENS_DATA_12   0x55   // R  #define MPU6050_EXT_SENS_DATA_13   0x56   // R  #define MPU6050_EXT_SENS_DATA_14   0x57   // R  
...
{      
  ...
  while (counter < 100) {
    ...
    update(result, (int)accel_t_gyro.value.y_gyro);
  ...
  Serial.print(", ");
  Serial.print(result[1]); // STD
  Serial.print(", ");
  Serial.println(result[2]); // COUNTER
}
...
void loop()
{
  ...
  press_1 =  analogRead(0);
  press_2 =  analogRead(1);
  press_3 =  analogRead(2);
  press_4 =  analogRead(3);
  ...
  */
  ...
  if (closing) {
      ...
      tmp[openCounter++] = storeMsg(index++, (int)accel_t_gyro.value.x_accel, (int)accel_t_gyro.value.y_accel, (int)accel_t_gyro.value.z_accel, (int)accel_t_gyro.value.x_gyro, (int)accel_t_gyro.value.y_gyro, (int)accel_t_gyro.value.z_gyro, press_1, press_2, press_3, press_4); 
  }
  else if(detect(result[0], result[1], (int)accel_t_gyro.value.y_gyro)) {
      tmp[openCounter++] = storeMsg(index++, (int)accel_t_gyro.value.x_accel, (int)accel_t_gyro.value.y_accel, (int)accel_t_gyro.value.z_accel, (int)accel_t_gyro.value.x_gyro, (int)accel_t_gyro.value.y_gyro, (int)accel_t_gyro.value.z_gyro,(int)press_1, press_2, press_3, press_4); 
      opened = true;
  }
  else {
      // Updating mean
      update(result, (int)accel_t_gyro.value.y_gyro);
      opened = false;
      freeMsg(tmp, openCounter);
      openCounter = 0;
      index = 0;
  }

  // Collect data 200 data if number of door open over 10 times continually.
  if (openCounter >= 10) {
      int i = 0;
     
      // Dump the previous ten continues data.
      while (i < 10) {
      ...
      while (!closing) {     
          ...
          if (detect(result[0], result[1], accel_t_gyro.value.y_gyro) && accel_t_gyro.value.y_gyro < 0) {
              closeCounter +=1;
          } else if (closeCounter >= 15) {
              closing = true;
          } else {
              closeCounter = 0;
          }
...
char *dump(int count, int acc_x, int acc_y, int acc_z, int gyro_x, int gyro_y, int gyro_z, int press_1, int press_2, int press_3, int press_4) {
    char *buf = (char*)malloc(sizeof(char)*50);
    sprintf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", count, acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z, press_1, press_2, press_3, press_4);
    
    return buf;
}

// Cached the 10 continues instances.
char *storeMsg(int count, int acc_x, int acc_y, int acc_z, int gyro_x, int gyro_y, int gyro_z, int press_1, int press_2, int press_3, int press_4) {
    char *buf = (char*)malloc(sizeof(char)*50);
    sprintf(buf, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", count, acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z, press_1, press_2, press_3, press_4);
...
// Free the 10 continues instances.
void freeMsg(char **msg, int count) {
    int i = 0;
    while (i < count) {
       free(msg[i++]); 
    }
...
/*
 * Online updating mean
 *
 * First parameter: Mean
 * Second parameter: Standard deviation
 * Third parameter: Number of data count
 */
void update(float *result, int newbie) {
    float mean = result[0];
    float std = result[1];
    int num = result[2];
    
    result[0] = (mean * num + newbie) / (num + 1);
    result[1] = sqrt(((std * std + mean * mean) * num + newbie * newbie) / (num + 1) - result[0] * result[0]);
    result[2] = num + 1;
}

// Detect the data if large than the bound.
boolean detect(float mean, float std, int newbie) {
    if ((newbie > (mean + 3 * std)) || (newbie < (mean - 3 * std))) 
       return true;
    else 
       return false;  
}
